...
var toolList = []map[string]any{
	{
		...
		"description": "Vet an open-source package before adopting or installing it. " +
			...
			"Returns ADOPT, CAUTION, or REJECT with per-signal reasons.",
...
func handle(req *request) *response {
	...
	case "initialize":
		...
		return result(req.ID, map[string]any{
			...
			"serverInfo":      map[string]any{"name": "depmesh-ai", "version": "0.1.0"},
		})
	case "ping":
		return result(req.ID, map[string]any{})
	case "tools/list":
...
func callTool(req *request) *response {
	...
	text, isError := runVet(params.Arguments.Ecosystem, params.Arguments.Package)
	return result(req.ID, map[string]any{
...
func runVet(ecosystem, name string) (text string, isError bool) {
	if ecosystem == "" || name == "" {
		return "Invalid arguments: both 'ecosystem' and 'package' are required", true
	}
	verdict, err := vet.Vet(ecosystem, name, true)
